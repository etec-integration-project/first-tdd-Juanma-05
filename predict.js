import dataset from "../ai/dataset.txt"
import autocorrect_model from "../entrenamiento/autocorrect_model (4).json"
...
let max_len = 10;
let words = [];
let model = create_model(max_len,ALPHA_LEN);
let status = "";
...
function setup(){
  ...
  .addEventListener('click',async ()=>{
    ...
    try{
      let filtered_words = preprocessing_stage_1(words,max_len);
      let int_words = preprocessing_stage_2(filtered_words,max_len);
      let train_features = preprocessing_stage_3(int_words,max_len,sample_len);
      let train_labels = preprocessing_stage_4(int_words,max_len,sample_len);
      ...
      await model.save(autocorrect_model); 
      //memory management
